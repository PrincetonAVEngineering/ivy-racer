...
try:
    while True:
        ...
        """# Merge co-linear contours
        for i in range(len(contours)):
            if len(contours[i]) < 2:  # Skip if contour too small
                continue
            for j in range(i + 1, len(contours)):
                if len(contours[j]) < 2:  # Skip if contour too small
                    continue
                # Fit lines to both contours
                line1 = cv2.fitLine(contours[i], cv2.DIST_L2, 0, 0.01, 0.01)
                line2 = cv2.fitLine(contours[j], cv2.DIST_L2, 0, 0.01, 0.01)
                
                # Calculate angles
                angle1 = np.arctan2(line1[1], line1[0]) * 180 / np.pi
                angle2 = np.arctan2(line2[1], line2[0]) * 180 / np.pi
                
                # If angles are similar (within 10 degrees) and contours are close
                if abs(angle1 - angle2) < 10:
                    # Merge contours by concatenating their points
                    contours[i] = np.concatenate((contours[i], contours[j]))
                    contours[j] = np.array([])  # Empty the merged contour"""
        # Create empty mask
        ...
        # After getting lines from HoughLinesP
        draw_lines(color_image, lines, color=(255, 0, 0))
        ...
        if p_lines is not None and len(p_lines) > 1:
            ...
            if det != 0:
                ...
                cv2.circle(color_image, (int(x), int(y)), 10, (0, 0, 255), -1)

        draw_lines(color_image, p_lines, color=(0, 255, 0))
